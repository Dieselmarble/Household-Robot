...
void XtarkBase::cmd_vel_callback(const geometry_msgs::Twist::ConstPtr& msg)
{
    try
    {
        ...
        last_vx = current_twist_.linear.x;
        last_wz = current_twist_.angular.z;
        current_twist_ = *msg.get();
        current_twist_.linear.x = 0.5 * last_vx + 0.5 * current_twist_.linear.x;
        current_twist_.angular.z = 0.5 * last_wz + 0.5 * current_twist_.angular.z;
