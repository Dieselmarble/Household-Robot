...
class RobotSelfCheck():
    def __init__(self):
        ...
        rospy.Subscriber('/xtark/lvel', Int32, self.avel_sub)
        rospy.Subscriber('/xtark/rvel', Int32, self.bvel_sub)
        rospy.Subscriber('/imu_raw', Imu, self.imu_sub)
        rospy.Subscriber('/battery_voltage', Float32, self.voltage_sub)
    ...
    def start_selfCheck(self):
        ...
        rospy.wait_for_message('/battery_voltage',Float32,3)
        if(self.voltage > 11):
        ...
        try:
            rospy.wait_for_message('/camera/image_raw/compressed',CompressedImage,2)
        ...
        try:
            rospy.wait_for_message('/scan',LaserScan,3)
        ...
        vel_t.start()
        sleep(1.0)
        print('-----MotorL Forward Check'+'\033[32m'+' OK!'+'\033[0m')
        print('-----MotorR Forward Check'+'\033[32m'+' OK!'+'\033[0m')
        sleep(3.0)
        vel_t = threading.Thread(target=self.pub_vel,args=(-0.5,)) 
        vel_t.setDaemon(True)
        vel_t.start()
        sleep(1.0)
        print('-----MotorL Backward Check'+'\033[32m'+' OK!'+'\033[0m')
        print('-----MotorR Backward Check'+'\033[32m'+' OK!'+'\033[0m')
