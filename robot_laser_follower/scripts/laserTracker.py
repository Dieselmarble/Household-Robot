...
class laserTracker:
	def __init__(self):
		...
		self.deltaDist = rospy.get_param('~deltaDist')
                #self.lidar_type = rospy.get_param('~lidarType')
		...
		self.infoPublisher = rospy.Publisher('/object_tracker/info', StringMsg, queue_size=2)
	...
	def registerScan(self, scan_data):
		...
		if(minDistance > scan_data.range_max):
			...
			#    minDistanceAngle = -(scan_data.angle_min + (minDistanceID-250) * scan_data.angle_increment)
                            #print(minDistanceID,' ',minDistanceAngle)
