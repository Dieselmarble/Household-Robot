...
class Follower:
		...
		# as soon as we stop receiving Joy messages from the ps3 controller we stop all movement:
                ...
                self.switchMode= rospy.get_param('~switchMode', 1) # if this is set to False the O button has to be kept pressed in order for it to move
                self.max_linear_speed = rospy.get_param('~maxLinearSpeed', 0.2) 
                self.max_angular_speed = rospy.get_param('~maxAngularSpeed', 0.2) 
        ...
        def positionUpdateCallback(self, position):
                ...
                angularSpeed = -np.clip(-uncliped_ang_speed, -self.max_angular_speed, self.max_angular_speed)
                linearSpeed  = np.clip(-uncliped_lin_speed, -self.max_linear_speed, self.max_linear_speed)
