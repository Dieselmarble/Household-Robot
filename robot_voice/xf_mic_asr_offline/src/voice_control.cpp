...
*/
		...
		ROS_INFO(whole_result);
		ROS_INFO_STREAM(whole_result);

		if (whole_result!="")
		{	
			cout<<("begin analysing")<<endl;
			printf(">>>>>全部返回结果:　[ %s ]\n", whole_result);
			ROS_INFO(">>>>>>全部返回结果");
...
int main(int argc, char *argv[])
{
	...
	ndHandle.param("source_path", source_path, std::string("/home/xtark/ros_ws/src/xf_mic_asr_offline"));
	ndHandle.param("/appid", appid, std::string("c0cc27ad"));//appid，需要更换为自己的
	...
	speak_pub = n.advertise<std_msgs::String>("/speak",10);
