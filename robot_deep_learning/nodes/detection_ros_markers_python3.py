...
from rocon_std_msgs.msg import Strings
...
class DetectionBoxMarker:
    ...
    def update(self, detection_3d, timestamp, frame_number):
        ...
        self.box_2d = detection_3d['box_2d']
        self.faceID = detection_3d['faceID']
    ...
    def get_bounding_boxes(self):
        ...
        if self.box_2d is not None:
            x0, y0, x1, y1 = self.box_2d
            detection_box_width_pix = y1 - y0
            detection_box_height_pix = x1 - x0
            center_x = int((x0+x1)/2)
            center_y = int((y0+y1)/2)
            bounding_box.center.x = center_x
            bounding_box.center.y = center_y
            bounding_box.size_x = detection_box_width_pix
            bounding_box.size_y = detection_box_height_pix
            bounding_box.center.theta = 0
        return bounding_box

    def get_faceID(self):
        faceID_msg = Strings()
        if self.faceID is not None:                
            faceID_msg = self.faceID
        return faceID_msg
...
class DetectionBoxMarkerCollection:
    ...
    def get_ros_bounding_box_array(self):
        ros_boxes = BoundingBox2DArray()
        for key in self.collection:
            marker = self.collection[key]
            if marker.frame_number == self.frame_number:
                bounding_box = marker.get_bounding_boxes()
                if bounding_box is not None:                
                    ros_boxes.boxes.append(bounding_box)
                    # ros_boxes.header.stamp = self.timestamp
                    # ros_boxes.header.frame_id = self.frame_number
        return ros_boxes

    def get_ros_faceID_array(self):
        faceID_array = StringArray()
        for key in self.collection:
            marker = self.collection[key]
            if marker.frame_number == self.frame_number:
                faceID_msg = marker.get_faceID()
                if faceID_msg is not None:           
                    faceID_array.strings.append(faceID_msg)
                    # faceID_array.header.stamp = str(self.timestamp)
                    # faceID_array.header.frame_id = str(self.frame_number)
        return faceID_array
