...
class DetectionNode:
    ...
    def image_callback(self, ros_rgb_image, ros_depth_image, rgb_camera_info):
        ...
        detection_box_image = detection_box_image
        ...
        debug_output = False
        detections_2d, output_image = self.detector.apply_to_image(detection_box_image, draw_output=debug_output) # detections_2d: {'box':bounding_box, 'ypr':ypr, 'landmarks':landmarks}       
        if debug_output: 
        ...
        boxes = self.marker_collection.get_ros_bounding_box_array()
        faceID_array = self.marker_collection.get_ros_faceID_array()
        ...
        self.visualize_markers_pub.publish(marker_array) # publish marker array
        self.visualize_bounding_box_pub.publish(boxes) # publish bounding box
        if axes_array is not None: 
            self.visualize_axes_pub.publish(axes_array)
        if faceID_array is not None:
            self.faceID_pub.publish(faceID_array) # publish faceID results
    ...
    def main(self):
        ...
        self.rgb_topic_name = '/cam_2/color/image_raw' #'/camera/infra1/image_rect_raw'
        self.rgb_image_subscriber = message_filters.Subscriber(self.rgb_topic_name, Image)

        self.depth_topic_name = '/cam_2/aligned_depth_to_color/image_raw'
        self.depth_image_subscriber = message_filters.Subscriber(self.depth_topic_name, Image)

        self.camera_info_subscriber = message_filters.Subscriber('/cam_2/color/camera_info', CameraInfo)
        ...
        self.visualize_bounding_box_pub = rospy.Publisher('/' + self.topic_base_name + '/bounding_boxes', BoundingBox2DArray, queue_size=1)
        self.faceID_pub = rospy.Publisher('/' + self.topic_base_name + '/faceID', StringArray, queue_size=1)
